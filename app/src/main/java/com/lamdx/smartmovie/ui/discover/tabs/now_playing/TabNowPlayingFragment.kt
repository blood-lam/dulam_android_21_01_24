package com.lamdx.smartmovie.ui.discover.tabs.now_playing
 ...
 * Use the [TabNowPlayingFragment.newInstance] factory method to
 * create an instance of this fragment.
 */

@AndroidEntryPoint
class TabNowPlayingFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_tab_now_playing, container, false)
        movieListFragment =
            childFragmentManager.findFragmentById(R.id.tab_now_playing_list) as MovieListFragment
        movieListFragment?.movieArray = movies

        val swipeRefreshLayout = view.findViewById<SwipeRefreshLayout>(R.id.refresh_tab_now_playing)
    ...
    private fun refreshData(swipeRefreshLayout: SwipeRefreshLayout) {
        discoverViewModel.viewModelScope.launch {
            discoverViewModel.loadNowPlaying.collect { state ->
                when (state.status) {
